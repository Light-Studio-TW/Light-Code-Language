...
export default (complexTypes, addErrorContent) => {
  for (let i = 0; i < complexTypes.length; i++) {
    if (complexTypes[i+1] !== undefined && !relevance[complexTypes[i].type].includes(complexTypes[i+1].type)) {
      ...
      return { error: true, type: 'analysis', content: `<關鍵字> "連結" 的後面必須為一個 <容器>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath: __dirname, function: '{語法檢查器}', line: complexTypes[i].line }] }
    } else if (complexTypes[i].type === 'keyword' && complexTypes[i].value === '連結' && complexTypes[i+2] !== undefined) {
      return { error: true, type: 'analysis', content: `多出了一個 <${typesName[complexTypes[i+2].type]}>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i+2].start, end: complexTypes[i+2].end, path: [{ filePath: __dirname, function: '{語法檢查器}', line: complexTypes[i+2].line }] }
      ...
      return { error: true, type: 'analysis', content: `<關鍵字> "變數" 的後面必須為一個 <容器> 或 <物件>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath: __dirname, function: '{語法檢查器}', line: complexTypes[i].line }] }
    }
  }
}