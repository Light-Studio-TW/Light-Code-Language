...
export default (complexTypes, filePath, addErrorContent) => {
  for (let i = 0; i < complexTypes.length; i++) {
    ...
    } else if (complexTypes[i].type === 'keyword' && complexTypes[i].value === '變數' && (complexTypes[i+2] !== undefined && complexTypes[i+2].type !== 'operator' && complexTypes[i+2].value !== '=')) {
      return { error: true, type: 'analysis', content: `多出了一個 <${typesName[complexTypes[i+2].type]}>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i+2].start, end: complexTypes[i+2].end, path: [{ filePath, function: '{語法檢查器}', line: complexTypes[i+2].line }] }
    } else if (complexTypes[i].type === 'keyword' && complexTypes[i].value === '函數' && (complexTypes[i+1] === undefined || (complexTypes[i+1].type !== 'container' && complexTypes[i+1].type !== 'parameters'))) {
      return { error: true, type: 'analysis', content: `<關鍵字> "函數" 的後面必須為 <容器> <參數列> <區塊> 或 <參數列> <區塊>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath, function: '{語法檢查器}', line: complexTypes[i].line }] }
    } else if (complexTypes[i].type === 'keyword' && complexTypes[i].value === '函數' && (complexTypes[i+1] !== undefined && complexTypes[i+1].type === 'container') && (complexTypes[i+2] === undefined || complexTypes[i+2].type !== 'parameters' || complexTypes[i+3] === undefined || complexTypes[i+3].type !== 'chunk')) {
      return { error: true, type: 'analysis', content: `<關鍵字> "函數" 的後面必須為 <容器> <參數列> <區塊> 或 <參數列> <區塊>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath, function: '{語法檢查器}', line: complexTypes[i].line }] }
    } else if (complexTypes[i].type === 'keyword' && complexTypes[i].value === '函數' && (complexTypes[i+1] !== undefined && complexTypes[i+1].type === 'parameters') && (complexTypes[i+2] === undefined || complexTypes[i+2].type !== 'chunk')) {
      return { error: true, type: 'analysis', content: `<關鍵字> "函數" 的後面必須為 <參數列> <區塊> 或 <容器> <參數列> <區塊>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath, function: '{語法檢查器}', line: complexTypes[i].line }] }
    } else if (complexTypes[i].type === 'keyword' && complexTypes[i].value === '重複' && (complexTypes[i+1] === undefined || complexTypes[i+1].type !== 'parameters' || complexTypes[i+2] === undefined || complexTypes[+2].type !== 'chunk')) {
      return { error: true, type: 'analysis', content: `<關鍵字> "重複" 的後面必須為 <參數列> <區塊>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath, function: '{語法檢查器}', line: complexTypes[i].line }] }
    }
  }
}