...
export default (complexTypes, filePath, addErrorContent) => {
  for (let i = 0; i < complexTypes.length; i++) {
    if (complexTypes[i+1] !== undefined && !relevance[complexTypes[i].type].includes(complexTypes[i+1].type)) {
      ...
      return { error: true, type: 'analysis', content: `<關鍵字> "函數" 的後面必須為一個 <容器> 或 <參數列>${(addErrorContent === undefined) ? '' : addErrorContent}`, start: complexTypes[i].start, end: complexTypes[i].end, path: [{ filePath, function: '{語法檢查器}', line: complexTypes[i].line }] }
